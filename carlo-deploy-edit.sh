...
read_deploy_file() {
    if [[ ! -f "$DEPLOY_FILE" ]]; then
        warning "Arquivo deploy.sh não encontrado para $DOMAIN"
        log "Localização: $DEPLOY_FILE"
        echo "#!/bin/bash"
    ...
    # Ler conteúdo do arquivo sem incluir logs
    cat "$DEPLOY_FILE"
...
write_deploy_file() {
    ...
    if printf "%b" "$content" > "$DEPLOY_FILE"; then
        chmod 755 "$DEPLOY_FILE"
        log "Arquivo deploy.sh salvo com sucesso para $DOMAIN"
        log "Localização: $DEPLOY_FILE"
        if [[ -n "$backup_path" ]]; then
            log "Backup disponível em: $backup_path"
        fi
        success "Script de deploy atualizado"
    else
        ...
        if [[ -n "$backup_path" && -f "$backup_path" ]]; then
            if cp "$backup_path" "$DEPLOY_FILE"; then
                warning "Arquivo deploy.sh restaurado do backup"
            else
...
case "$ACTION" in
    "read")
        log "Lendo script de deploy para $DOMAIN"
        read_deploy_file
        ;;
    "write")
        ...
        log "Salvando script de deploy para $DOMAIN"
